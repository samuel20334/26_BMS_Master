...
void wakeup_idle(uint8_t total_ic) //Number of ICs in the system
{
	for (int i =0; i<total_ic; i++)
	{
	   ...
	   spi_read_byte(&hspi1, &dummy);//Guarantees the isoSPI will be in ready mode
				...
				)
{
	...
	cs_low(CS_PORT, CS_PIN);
	spi_write_array(&hspi1, 4, cmd);
	spi_read_array(&hspi1, (BYTES_IN_REG*total_ic), data);         //Transmits the command and reads the configuration data of all ICs on the daisy chain into rx_data[] array
	cs_high(CS_PORT, CS_PIN);
                     ...
                     )
{
	...
	cs_low(CS_PORT, CS_PIN);
	spi_write_read(&hspi1, cmd, data, (REG_LEN*total_ic));
	cs_high(CS_PORT, CS_PIN);
}
